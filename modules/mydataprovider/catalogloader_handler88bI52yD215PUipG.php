...
// $ time php -f catalogloader_handler.php request=prefix cmsName=prestashop
/*
if ($argc > 1 ) {
...
class PrestaShopExtractor extends CmsDataExtractor
{
    ...
    function GetDbSettings()
    {
        ...
        $this->_db->host = $config['parameters']['database_host'];
        $this->_db->port = $config['parameters']['database_port'];
        $this->_db->dbName = $config['parameters']['database_name'];
        $this->_db->user = $config['parameters']['database_user'];
        $this->_db->password = $config['parameters']['database_password'];
        $this->_db->dbPref = $config['parameters']['database_prefix'];
        */
...
class ImageDownloader
{
    ...
    function ImageDownloader($link, $name, $params, $cmsName, $requestString = null)
    {
        ...
        $this->_link = $link;
        if (!preg_match('/^http[s]?:\/\//', $link) && // $link Start With Http
            !preg_match('/^ftp[s]?:\/\//', $link)) // $link Start With Ftp
        {
            // TODO: replace "https" with current schema
            if (preg_match('/^:?\/\/\w+/', $link)) // $link Start With Protocol-Less Schema (http or https) for mixed content compatibility
            {
                if (preg_match('/^:/', $link))
                {
                    $this->_link = 'https' . $link;
                }
                else
                {
                    $this->_link = 'https:' . $link;
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public function CurlGet($url)
    {
        ...
        // Проблема: ali отдаёт фотки в JXR (Jpeg XR) для всех новых или непонятных браузеров
        $agent = 'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0)'; // IE8 - no support of XHR
        //$agent = 'Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko';  // IE11
        //$agent = 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.114 Safari/537.36'; // Chrome35
        
        $header[0] = "Accept: text/html, application/xhtml+xml, */*";
        $header[1] = "Accept-Language: en-US";
        // $header[2] = "Accept-Charset: ISO-8859-1,utf-8;q=0.7,*;q=0.7";
        $curlOptions = array(
            ...
            CURLOPT_FOLLOWLOCATION=>true, // этот форвард не всегда работает
            CURLOPT_MAXREDIRS=>20,
            CURLOPT_SSL_VERIFYPEER=>false,
            CURLOPT_HTTPHEADER=>$header,
            //CURLOPT_RETURNTRANSFER=>1
            //CURLOPT_REFERER, 'https://www.aliexpress.com'
        );
        ...
        {
            //$this->log->ToLog('status = ' . var_dump($status), 'info');
            $status_code = $status['http_code'];
            if($status_code == 301 || $status_code == 302) // для случаев, когда CURLOPT_FOLLOWLOCATION=>true не отработал
            {
                $url = $status['url'];
                if (empty($url))
                {
                    ...
                    //$this->log->ToLog('new url = ' . $url, 'info');
                }
                
                $url_parsed = parse_url($url);
                $this->log->ToLog('Redirect ' . $status_code . ' url =' . $url . ' url_parsed = ' . $url_parsed, 'info');
