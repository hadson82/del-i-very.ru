...
class ToolsModuleOCC
{
		...
		if (function_exists('error_get_last'))
		{
			...
			if ($error && $error['type'] === E_ERROR)
			{
				...
				if (preg_match($memory_regex, $message, $matches))
				{
					...
					$message .= ')';
					LoggerOCC::getInstance()->error($message);
					LoggerOCC::getInstance()->error(
				...
				die(Tools::jsonEncode(array(
					'hasError' => LoggerOCC::getInstance()->hasError(),
					'log' => LoggerOCC::getInstance()->getMessages()
				)));
			}
		}
		exit;
	...
	public static function createAjaxApiCall($class)
	{
		...
		if (method_exists($class, $call_method))
		{
			...
			{
				LoggerOCC::getInstance()->exception($e);
				die(Tools::jsonEncode(array(
					'hasError' => LoggerOCC::getInstance()->hasError(),
					'log' => LoggerOCC::getInstance()->getMessages()
				)));
	...
	public static function getPaymentMethods()
	{
		...
		$t = TransModOCC::getInstance();
		$default_payment_icon = _MODULE_DIR_.ToolsModuleOCC::getModNameForPath(__FILE__).'/views/img/default_payment_icon.png';
	...
	public static function addCSS($css_uri, $css_media_type = 'all', $offset = null, $check_path = true)
	{
		Context::getContext()->controller->addCSS(
			_MODULE_DIR_.self::getModNameForPath(__FILE__).'/views/css/'.$css_uri, $css_media_type, $offset, $check_path
		);
	}
