...
class BlogTools
{
	...
	public static function registerSmartyFunctions()
	{
		...
		if (!array_key_exists('classItemBootstrap', $smarty->registered_plugins['function']))
			smartyRegisterFunction($smarty, 'function', 'classItemBootstrap', array(__CLASS__, 'classItemBootstrap'));
	}
	...
	public static function classItemBootstrap($params)
	{
		$iteration = (int)$params['iteration'];
		$last = (int)$params['last'];
		$items = $params['items'];
		$big = (isset($params['big']) ? (int)$params['big'] : 3);
		$medium = (isset($params['medium']) ? (int)$params['medium'] : 4);
		$small = (isset($params['small']) ? (int)$params['small'] : 2);

		$class = '';

		if ($iteration % $big == 0)
			$class .= ' big_row_end_line ';

		if ($iteration % $medium == 0)
			$class .= ' middle_row_end_line ';

		if ($iteration % $small == 0)
			$class .= ' small_row_end_line ';

		if ((count($items) - $iteration) < (count($items) % $big) || count($items) <= $big)
			$class .= ' big_last_line ';

		if ((count($items) - $iteration) < (count($items) % $medium) || count($items) <= $medium)
			$class .= ' middle_last_line ';

		if ((count($items) - $iteration) < (count($items) % $small) || count($items) <= $small)
			$class .= ' small_last_line ';
		...
		if (function_exists('error_get_last'))
		{
			...
			if ($error && $error['type'] === E_ERROR)
			{
				...
				if (preg_match($memory_regex, $message, $matches))
				{
					...
					$message .= ')';
					BlogLogger::getInstance()->error($message);
					BlogLogger::getInstance()->error($l->l('Your web-server is too slow, not enough RAM.', __FILE__));
					BlogLogger::getInstance()->error($l->l('Try to reduce some of expert\'s settings.', __FILE__));
				}
				elseif (preg_match($time_regex, $message, $matches))
				{
					...
					BlogLogger::getInstance()->error($message);
					BlogLogger::getInstance()->error($l->l('Your web-server is too slow, increase PHP execution time limit.', __FILE__));
					BlogLogger::getInstance()->error($l->l('Try to reduce some of expert\'s settings.', __FILE__));
				}
				else
					BlogLogger::getInstance()->error($message);
				...
				die(Tools::jsonEncode(array(
					'log' => BlogLogger::getInstance()->getMessages()
				)));
			}
		}
		exit;
	...
	public static function createAjaxApiCall($class)
	{
		...
		if (method_exists($class, $call_method))
		{
			...
			{
				BlogLogger::getInstance()->exception($e);
				die(Tools::jsonEncode(array(
					'log' => BlogLogger::getInstance()->getMessages()
				)));
			}
