...
class HelperDbSMT
{
	...
	public function definitionFieldToSQL($field_name, $definition)
	{
		...
		return '`'.bqSQL($field_name).'` '.$sql_type
			.' '.($sql_type != 'text' ? $this->getSQLDefaultVal($field_name,
	...
	private function getAllLang()
	{
		...
		ON b.`'.bqSQL($this->definition['primary']).'` = a.`'.bqSQL($this->definition['primary']).'` AND b.`id_lang` = '.(int)Context::getContext()->language->id);
	}
	...
	{
		return Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.bqSQL($this->definition['table']).'` WHERE `'.bqSQL($this->definition['primary']).'` = '.(int)$id);
	}
