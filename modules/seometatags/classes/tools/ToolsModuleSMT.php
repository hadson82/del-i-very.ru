...
class ToolsModuleSMT
{
	...
	public static function registerSmartyFunctions()
	{
        ...
        if (!array_key_exists('pluralForm', $smarty->registered_plugins['modifier']))
			smartyRegisterFunction($smarty, 'modifier', 'pluralForm', array(__CLASS__, 'pluralForm'));
		if (class_exists('TransModSMT'))
		{
			if (!array_key_exists('ld', $smarty->registered_plugins['modifier']))
				smartyRegisterFunction($smarty, 'modifier', 'ld', array(TransModSMT::getInstance(), 'ld'));
		}
		if (!array_key_exists('dt', $smarty->registered_plugins['modifier']))
			smartyRegisterFunction($smarty, 'modifier', 'dt', array(__CLASS__, 'dateFormatTranslate'));
	}
		...
		if (function_exists('error_get_last'))
		{
			...
			if ($error && $error['type'] === E_ERROR)
			{
				...
				if (preg_match($memory_regex, $message, $matches))
				{
					...
					$message .= ')';
					LoggerSMT::getInstance()->error($message);
					LoggerSMT::getInstance()->error(
				...
				die(Tools::jsonEncode(array(
					'hasError' => LoggerSMT::getInstance()->hasError(),
					'log' => LoggerSMT::getInstance()->getMessages()
				)));
			}
		}
		exit;
	...
	public static function createAjaxApiCall($class)
	{
		...
		if (method_exists($class, $call_method))
		{
			...
			{
				LoggerSMT::getInstance()->exception($e);
				die(Tools::jsonEncode(array(
					'hasError' => LoggerSMT::getInstance()->hasError(),
					'log' => LoggerSMT::getInstance()->getMessages()
				)));
	...
	public static function getPaymentMethods()
	{
		...
		$t = TransModSMT::getInstance();
		$default_payment_icon = _MODULE_DIR_.ToolsModuleSMT::getModNameForPath(__FILE__).'/views/img/default_payment_icon.png';
	...
	{
		Context::getContext()->controller->addCSS(_MODULE_DIR_.self::getModNameForPath(__FILE__).'/views/css/'.$css_uri, $css_media_type, $offset, $check_path);
	}
