...
<div itemscope itemtype="https://schema.org/Product">
		...
		<!-- product img-->
			...
			{if $have_image}
				<span id="view_full_size">
					...
					<a id="zoom1" rel="position: 'inside' , showTitle: false, adjustX:0, adjustY:0" class="cloud-zoom" href="{$link->getImageLink($product->link_rewrite, $cover.id_image,'thickbox_default')|escape:'html'}">
						<img itemprop="image" id="mousetrap_img" alt="{$product->name|escape:'htmlall':'UTF-8'}" width="106" height="106" title="{$product->name|escape:'htmlall':'UTF-8'}" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_default')|escape:'html'}" >
						<img itemprop="image" id="bigpic" alt="{$product->name|escape:'htmlall':'UTF-8'}" title="{$product->name|escape:'htmlall':'UTF-8'}" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html'}" />
						<span class="mask"></span>
					</a>
					{else}
					<img itemprop="image" id="bigpic" alt="{$product->name|escape:'htmlall':'UTF-8'}" title="{$product->name|escape:'htmlall':'UTF-8'}" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')}" />
				{/if}
				</span>
			{else}
				<span id="view_full_size">
					...
					{if isset($images)}
						...
						<li id="thumbnail_{$image.id_image}">
							...
							<a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html'}" class="cloud-zoom-gallery" title="{$imageTitlte}" rel="useZoom: 'zoom1', smallImage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html'}'">
								<img itemprop="image" id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_default')|escape:'html'}" alt="{$imageTitlte}"  />
							</a>
							{else}
			...
			{/if}
			<div class="ya-share2 yandex-share" data-services="whatsapp,viber,vkontakte,odnoklassniki,facebook"></div>
			...
			<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0} style="display:none;"{/if} action="{$link->getPageLink('cart')|escape:'html'}" method="post">
				...
				<p class="warning_inline" id="last_quantities"{if ($product->quantity > $last_qties OR $product->quantity <= 0) OR $allow_oosp OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Warning: Last items in stock!'}</p>
					...
					<p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
						...
						<span class="editable" itemprop="sku"{if !empty($product->reference) && $product->reference} content="{$product->reference}"{/if}>{if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}
						</span>
					</p>
										...
										<li{if $group.default == $id_attribute} class="selected"{/if}>
											<a id="color_{$id_attribute|intval}" class="color_pick{if ($group.default == $id_attribute)} selected{/if}" style="background: {$colors.$id_attribute.value};" title="{$colors.$id_attribute.name}" onclick="colorPickerClick(this);getProductAttribute();">
												{if file_exists($col_img_dir|cat:$id_attribute|cat:'.jpg')}
													<img src="{$img_col_dir}{$id_attribute}.jpg" alt="{$colors.$id_attribute.name}" width="25" height="25" /> 
												{/if}
											</a>
										</li>
	...
	<section class="page_product_box toggle_frame more_info_inner">
		<h2 class="toggle">{l s='More info'}</h2>
			<div>
			...
			{if isset($product) && $product->description}
				<div class="toggle_content prod_des">
					<div class="row">
						<div class="col-xs-12">
							{$product->description}
						</div>
	...
	<div class="row productyellowtextcategory">
			...
			<div class="customization_block toggle_content">
				<form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm" class="clearfix">
					<p class="infoCustomizable">
						...
						{if $product->uploadable_files} {l s='Allowed file formats are: GIF, JPG, PNG'}{/if}
					</p>
    ...
    <section class="page_product_box toggle_frame more_info_inner">
        <h2 class="toggle">Отзывы о товаре {$product->name|escape:'htmlall':'UTF-8'}</h2>
        <!-- Код виджета комментариев {literal} -->
        <div id="hypercomments_widget"></div>
        <script type="text/javascript">
        _hcwp = window._hcwp || [];
        _hcwp.push({widget:"Stream", widget_id: 81093});
        (function() {
