...
<div id="featured-products_block_center" class="main_page_products block">
	<h4><span>{l s='Рекомендуемые товары' mod='homefeatured'}</span></h4>
	{if isset($products) AND $products}
			...
			{foreach from=$products item=product name=homeFeaturedProducts}
                ...
                {if isset($product.reduction) && $product.reduction && isset($product.on_sale) && $product.on_sale && isset($product.show_price) && $product.show_price && !$PS_CATALOG_MODE}<div class="sale-box"><span class="before"></span><span class="after"></span><span class="on_sale title_shop">{l s='On sale!'}</span></div>{/if}
                    ...
                    {/if}
					<h5><a class="product_link" href="{$product.link|escape:'html'}" title="{$product.name|truncate:50:'...'|escape:'htmlall':'UTF-8'}">{$product.name|truncate:150:'...'|escape:'htmlall':'UTF-8'}</a></h5>
