...
<section class="block blockcms_footer col-sm-3">
		...
		{*{if !$PS_CATALOG_MODE}<li class="first_item"><a href="{$link->getPageLink('prices-drop')|escape:'html'}" title="{l s='Specials' mod='blockcms'}">{l s='Specials' mod='blockcms'}</a></li>{/if}
			...
			{*{foreach from=$cmslinks item=cmslink}
				...
				{/if}
			{/foreach}*}
			<li><a href="{$link->getCMSLink('6', 'CMSpage')|escape:'html'}" title="{l s='contacts' mod='blockcms'}">{l s='Контакты' mod='blockcms'}</a></li>
			<li><a href="{$link->getCMSLink('9', 'CMSpage')|escape:'html'}" title="{l s='contacts' mod='blockcms'}">{l s='Сборка мебели ИКЕА' mod='blockcms'}</a></li>
			<li><a href="{$link->getPageLink('sitemap')|escape:'html'}" title="{l s='sitemap' mod='blockcms'}">{l s='Sitemap' mod='blockcms'}</a></li>
			<li><a href="{$link->getCMSLink('13', 'CMSpage')|escape:'html'}" title="{l s='contacts' mod='blockcms'}">{l s='Политика конфиденциальности' mod='blockcms'}</a></li>


		</ul>
	{$footer_text}
</section>
{if $display_poweredby}<section class="bottom_footer">
<div class="container">&copy; 2013 &mdash; {$smarty.now|date_format:"%Y"} | Интернет-магазин официальных товаров ИКЕА del-i-very.ru</div>		
</section>{/if}
	<!-- /MODULE Block footer -->
{/if}
